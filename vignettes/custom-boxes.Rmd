...
- **box.icon**: HTML code which will be prepended to the `box.title` to render an icon. You can use a package like [icon](https://github.com/ropenscilabs/icon) to generate the code.
...
Example of a coloured box using an icon:
    ...
    `r ''````{block, box.title = "Title and icon", box.body = list(fill = "lightblue"), box.icon = icon::ii_star()}
    This box contains an icon in the header...
    ```

```{block, box.title = "Title and icon", box.body = list(fill = "lightblue"), box.icon = icon::ii_star()}
This box contains an icon in the header...
```
