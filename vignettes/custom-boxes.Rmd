...
- **box.body**: A list containing `fill` (background) and/or `colour` (text if applicable _e.g._ block chunk) to define the colours of the body. Use the _R_ color specifications (see `grDevices::col2rgb()`).
- **box.header**: A list containing `fill` and/or `colour` to define the colours of the header.
- **box.collapse**: The collapsible status of the HTML box.
    + `NULL`: the box is not collapsible
    + `TRUE`: the box is collapsed
    + `FALSE`: the box is not collapsed but collapsible
- **box.icon**: HTML code which will be prepended to the `box.title` to render an icon. You can use a package like [icon](https://github.com/ropenscilabs/icon) to generate the code and manage the library dependency.
...
Example of a coloured box using an icon:
    ...
    `r ''````{block, box.title = "Title and icon", box.body = list(fill = "lightblue"), box.icon = icon::fa_star()}
    This box contains an icon in the header...
    ```

```{block, box.title = "Title and icon", box.body = list(fill = "lightblue"), box.icon = icon::fa_star()}
This box contains an icon in the header...
```
...
knitr::opts_template$set(alert = list(box.title = "Watch out!",
                                      ...
                                      box.collapse = TRUE,
                                      box.icon = icon::fa_exclamation_triangle()))
```
...
```{r, include = FALSE}
# Let fa work with pkgdown: boostrap uses fa v4.7 while icon already uses v5
# Added a .fas css class to extra.css
# flush fa icon dependency introduced by the icon package: pkgdown already loads fa
knitr::knit_meta()
```